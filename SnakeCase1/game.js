...
let G = (function (){
	...
	let resetSnake = function(data){
		...
		deleteSnakePart(snakeLine.length);
		snakeLine = [];
		pivotLine = [];
		snakeDistance = 0;
		updateUI(snakeLength);
	...
	let pivotSnake = function(x,y){
		//Save the last line so we can delete it later
		allSnakeLines.push(snakeLine);
		snakePivotX = x;
		snakePivotY = y;
	}
